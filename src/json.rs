...
// Finds the index of needle in the slice, respecting escape sequences
//
// This method is used to find:
//  1. the next occurrence of '/' when splitting ref tokens in pointer
//  2. closing quotes, either single or double when parsing name selectors
//
// It does not care if the escaped sequence is valid. When we do p + 2, we skip '\' plus the escaped
// character; For our use case, any of the '/', '\'', '"' will be skipped correctly. Later the parser
// will look at the sequence and report that is invalid. This blind skip approach would be problematic
// if the characters that we search for appear in a Unicode sequence. To skip a Unicode sequence
// correctly we need + 6 but + 2 where we skip '\u' works.
fn find_unescaped(haystack: &[u8], needle: u8) -> Option<usize> {
    let mut pos = 0;
    let len = haystack.len();
    while pos < len {
        // memchr2 returns the position relative to the slice not the entire buffer so we need to
        // adjust and get the absolute position when we index into the buffer
